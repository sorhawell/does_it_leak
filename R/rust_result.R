...
unwrap = function(result, context = NULL, call = sys.call(1L)) {
  if (!is_result(result)) stopf("Internal error: Wrong type, cannot unwrap non-result")
  if (is_ok(result)) return(result$ok)
  if (is_err(result)) {
    if (!is.null(context)) {
      result = result |> map_err(\(err) paste(context, err))
    }
    stop(
      paste(
        result$err,
  ...
  stopf("Internal error: failed to unwrap a result")
}
